...
def write_schema(catalog, stream_name):
    ...
    try:
        if stream_name not in WRITED_SCHEMA:
            singer.write_schema(stream_name, schema, stream.key_properties)
            WRITED_SCHEMA.append(stream_name)
...
def get_bookmark(state, stream, default, account_id):
...
def write_bookmark(state, stream, value, account_id):
    if 'bookmarks' not in state:
        state['bookmarks'] = {}
    if account_id not in state['bookmarks']:
        state['bookmarks'][account_id] = {}
    state['bookmarks'][account_id][stream] = value
...
def sync_endpoint(client,
    ...
    # Bookmark datetimes
    last_datetime = get_bookmark(state, stream_name, start_date, account_id)
    ...
    if bookmark_field:
        write_bookmark(state, stream_name, max_bookmark_value, account_id)
    ...
    for placement in PLACEMENTS: # ALL_ON_TWITTER, PUBLISHER_NETWORK
        ...
        if entity_ids != []:
            ...
            if report_granularity == 'DAY':
                min_start_str = min_start_rounded.strftime('%Y-%m-%d')
                max_end_str = max_end_rounded.strftime('%Y-%m-%d')
            else:
                min_start_str = min_start_rounded.strftime('%Y-%m-%dT%H:%M:%S%z')
                max_end_str = max_end_rounded.strftime('%Y-%m-%dT%H:%M:%S%z')

            entity_id_set = {
                'placement': placement,
                'entity_ids': entity_ids,
                'start_time': min_start_str,
                'end_time': max_end_str
            }
...
def sync_report(client,
    ...
    # Bookmark datetimes
    last_datetime = get_bookmark(state, report_name, start_date, account_id)
    ...
    while window_start != abs_end:
        ...
        for sub_type_id in sub_type_ids:
            ...
            for entity_id_set in entity_id_sets:
                ...
                # In some rare cases, times are in the wrong order.
        ...
        write_bookmark(state, report_name, max_bookmark_value, account_id)
