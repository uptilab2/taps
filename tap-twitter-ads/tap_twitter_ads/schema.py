...
def resolve_schema_references(schema, refs):
    for property in schema['properties']:
        ...
        if '$ref' in schema['properties'].get(property):
            link = schema['properties'].get(property).get('$ref')
            if link:
                child_schema = resolve_schema_references(refs[link], refs)
                schema['properties'][property].update(child_schema)
                del schema['properties'][property]['$ref']
    return schema
