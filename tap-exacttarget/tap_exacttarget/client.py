...
def get_auth_stub(config):
    ...
    try:
        ...
        transport = HttpAuthenticated(timeout=int(config.get('request_timeout', 7200)))
        auth_stub.soap_client.set_options(
            transport=transport, timeout=int(config.get('request_timeout', 7200)))
        LOGGER.info("Success.")
        return auth_stub
...
def request_from_cursor(name, cursor, batch_size):
    ...
    while response.more_results:
        ...
        response = tap_exacttarget__getMoreResults(cursor, batch_size)
